...
from ecommerce.certificados.views import EmissaoNv1WizardView, EmissaoNv3WizardView, EscolhaVoucherView, \
    ...
    RevisaoEmissaoNv4WizardView, RevisaoEmissaoNvAWizardView, RevisaoEmissaoNvBWizardView, VouchersPendentesListView, AprovaVoucherPendenteView, CodigoSeloView, ChavePublicaView, ReenviaVoucherComodoView
...
urlpatterns = patterns(
    ...
    url(r'^emissao/(%s|%s)/(?P<crm_hash>[\w_-]+)/$' % (Voucher.PRODUTO_SSL, Voucher.PRODUTO_SSL_WILDCARD), login_required(EmissaoNv1WizardView.as_view(form_list=FORMS_NV1)), name='form-emissao-nv1'),
    url(r'^emissao/(%s|%s)/(?P<crm_hash>[\w_-]+)/$' % (Voucher.PRODUTO_SAN_UCC, Voucher.PRODUTO_MDC), login_required(EmissaoNv2WizardView.as_view(form_list=FORMS_NV2)), name='form-emissao-nv2'),
    url(r'^emissao/(%s)/(?P<crm_hash>[\w_-]+)/$' % Voucher.PRODUTO_EV, login_required(EmissaoNv3WizardView.as_view(form_list=FORMS_NV3)), name='form-emissao-nv3'),
    url(r'^emissao/(%s)/(?P<crm_hash>[\w_-]+)/$' % Voucher.PRODUTO_EV_MDC, login_required(EmissaoNv4WizardView.as_view(form_list=FORMS_NV4)), name='form-emissao-nv4'),
    url(r'^emissao/(%s|%s)/(?P<crm_hash>[\w_-]+)/$' % (Voucher.PRODUTO_CODE_SIGNING, Voucher.PRODUTO_JRE), login_required(EmissaoNvAWizardView.as_view(form_list=FORMS_NVA)), name='form-emissao-nvA'),
    url(r'^emissao/(%s)/(?P<crm_hash>[\w_-]+)/$' % Voucher.PRODUTO_SMIME, login_required(EmissaoNvBWizardView.as_view(form_list=FORMS_NVB)), name='form-emissao-nvB'),

    url(r'^revisao-emissao/(%s|%s)/(?P<crm_hash>[\w_-]+)/$' % (Voucher.PRODUTO_SSL, Voucher.PRODUTO_SSL_WILDCARD), login_required(RevisaoEmissaoNv1WizardView.as_view(form_list=FORMS_NV1)), name='form-revisao-emissao-nv1'),
    url(r'^revisao-emissao/(%s|%s)/(?P<crm_hash>[\w_-]+)/$' % (Voucher.PRODUTO_SAN_UCC, Voucher.PRODUTO_MDC), login_required(RevisaoEmissaoNv2WizardView.as_view(form_list=FORMS_NV2)), name='form-revisao-emissao-nv2'),
    url(r'^revisao-emissao/(%s)/(?P<crm_hash>[\w_-]+)/$' % Voucher.PRODUTO_EV, login_required(RevisaoEmissaoNv3WizardView.as_view(form_list=FORMS_NV3)), name='form-revisao-emissao-nv3'),
    url(r'^revisao-emissao/(%s)/(?P<crm_hash>[\w_-]+)/$' % Voucher.PRODUTO_EV_MDC, login_required(RevisaoEmissaoNv4WizardView.as_view(form_list=FORMS_NV4)), name='form-revisao-emissao-nv4'),
    url(r'^revisao-emissao/(%s|%s)/(?P<crm_hash>[\w_-]+)/$' % (Voucher.PRODUTO_CODE_SIGNING, Voucher.PRODUTO_JRE), login_required(RevisaoEmissaoNvAWizardView.as_view(form_list=FORMS_NVA)), name='form-revisao-emissao-nvA'),
    url(r'^revisao-emissao/(%s)/(?P<crm_hash>[\w_-]+)/$' % Voucher.PRODUTO_SMIME, login_required(RevisaoEmissaoNvBWizardView.as_view(form_list=FORMS_NVB)), name='form-revisao-emissao-nvB'),

    url(r'^revogacao/(?P<crm_hash>[\w_-]+)/$', login_required(RevogacaoView.as_view()), name='form-revogacao'),

    url(r'^reemissao/(?P<crm_hash>[\w_-]+)/$', login_required(ReemissaoView.as_view()), name='form-reemissao'),
    ...
    url(r'^aprova-vouchers-pendente/(?P<crm_hash>[\w_-]+)/$', login_required(AprovaVoucherPendenteView.as_view()), name='aprova-voucher-pendente'),
    url(r'^reenvia-voucher-comodo/(?P<crm_hash>[\w_-]+)/$', login_required(ReenviaVoucherComodoView.as_view()), name='reenvia-voucher-comodo'),
    url(r'^codigo-do-selo/(?P<crm_hash>[\w_-]+)/$', login_required(CodigoSeloView.as_view()), name='codigo-selo'),
    url(r'^chave-publica/(?P<crm_hash>[\w_-]+)/$', login_required(ChavePublicaView.as_view()), name='chave-publica'),
