...
class Migration(SchemaMigration):
    ...
    def backwards(self, orm):
        if db.backend_name != 'sqlite3':
            db.drop_foreign_key(u'certificados_revogacao', 'emission_id')
        db.rename_column(u'certificados_revogacao', 'emission_id', 'emissao_id')
        db.rename_column(u'certificados_revogacao', 'revoke_reason', 'revogacao_motivo')
        if db.backend_name != 'sqlite3':
            db.alter_column(u'certificados_revogacao', 'emissao_id', models.ForeignKey(to=orm['certificados.Emissao']))
